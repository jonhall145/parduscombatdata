...
shipInfo.ECM = "none";
shipInfo.ECCM = "none";
var serverURL = "https://asdwolf.com/combat_data_handler_beta.php";
var combatToSubmit = [];
var cr_interpreted = [];
var cr_raw = [];
...
function loadCombatLog(combatLink) {
    GM_xmlhttpRequest({
        ...
        onload: function(response) {
            ...
            updateSkillStatButton(cr_interpreted, cr_raw);
        }
    });
    updateSkillStatButton(cr_interpreted, cr_raw);
...
function loadShipInfo(combatStats, skillsToSend, cr_raw) {
     GM_xmlhttpRequest({
        method: "GET",
        url: shipLink,
        onload: function(response) {
            ...
            submitToServer(combatStats, skillsToSend, shipInfo, cr_raw);

        }
     });
}

function loadCombatSkills(combatStats, cr_raw) {
    GM_xmlhttpRequest({
        method: "GET",
        url: skillsLink,
        onload: function(response) {
            ...
            loadShipInfo(combatStats, skillsToSend, cr_raw);
            //submitToServer(combatStats, skillsToSend, shipInfo);
        }
    });
}

function submitToServer(combatStats, skills, shipInfo, cr_raw) {
...
function updateSkillStatButton(combatToSubmit, cr_raw) {
    skillstatbutton.value = "load stats and submit"
    skillstatbutton.onclick = function() {
        loadCombatSkills(combatToSubmit, cr_raw);
